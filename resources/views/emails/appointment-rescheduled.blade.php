...
<div class="container">
  ...
  <div class="body">
    <p>Dear <strong>{{ $appointment->customer->first_name ?? 'Customer' }}</strong>,</p>
    ...
    <div class="info-box">
      <p><strong>Vehicle:</strong>
        ...
        ({{ strtoupper($appointment->vehicle->registration_number ?? '') }})
      </p>
      <p><strong>Service:</strong> {{ ucwords(str_replace('-', ' ', $appointment->appointment_type ?? 'Service')) }}</p>
      <p><strong>Proposed New Date:</strong> {{ \Carbon\Carbon::parse($appointment->proposed_date)->format('l, d F Y') }}</p>
      <p><strong>Proposed New Time:</strong> {{ \Carbon\Carbon::parse('2000-01-01 ' . $appointment->proposed_time)->format('g:i A') }}</p>
    </div>

    <div class="original-box">
      <p style="margin:0;font-size:13px;color:#92400e;">
        <strong>Original booking:</strong>
        {{ $appointment->scheduled_date ? $appointment->scheduled_date->format('D, d M Y \a\t g:i A') : 'N/A' }}
      </p>
    </div>

    <p style="text-align:center;margin:28px 0 12px;">
      <a href="{{ $acceptUrl }}" class="btn btn-accept">&#10003; Accept New Time</a>
      <a href="{{ $declineUrl }}" class="btn btn-decline">&#10007; Decline / Keep Original</a>
    </p>

    <p style="font-size:13px;color:#666;">If the buttons above do not work, copy and paste these links into your browser:</p>
    <p style="font-size:12px;word-break:break-all;color:#2563eb;">Accept: {{ $acceptUrl }}</p>
    <p style="font-size:12px;word-break:break-all;color:#dc2626;">Decline: {{ $declineUrl }}</p>
  ...
  <div class="footer">
    &copy; {{ date('Y') }} Doyen Auto Services &mdash; Precision Vehicle Diagnostics &amp; Technical Solutions Centre
  </div>
</div>
</body>
</html>
