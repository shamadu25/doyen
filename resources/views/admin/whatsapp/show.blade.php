...
@section('title', 'Chat – ' . ($conversation->customer_name ?? $conversation->customer_phone))
...
<div class="breadcrumb">
    <a href="{{ route('whatsapp.support.index') }}">&#128172; WhatsApp Support</a>
    &rsaquo; {{ $conversation->customer_name ?? $conversation->customer_phone }}
</div>
...
<div class="chat-layout">
    ...
    <div class="chat-card">
        ...
        <div class="chat-header">
            <div class="avatar">
                {{ strtoupper(substr($conversation->customer_name ?? $conversation->customer_phone, 0, 1)) }}
            </div>
            <div class="info">
            ...
            @forelse($messages as $msg)
                ...
                <div class="msg {{ $msg->direction }}">
                    <div class="msg-bubble">
                        ...
                        @if($msg->hasMedia())
                            <div class="msg-media">
                                @if(str_starts_with($msg->media_content_type ?? '', 'image'))
                                    <img src="{{ $msg->media_url }}" alt="Media">
                                @else
                                    <a href="{{ $msg->media_url }}" target="_blank" style="color:#667eea;">&#128206; View Attachment</a>
                                @endif
                            </div>
                        @endif
                    </div>
                    <div class="msg-meta">
                        ...
                        @if($msg->isOutbound())
                            &middot; {{ $msg->sentBy?->name ?? 'System' }}
                            @if($msg->status === 'delivered') &middot; &#10004;&#10004;
                            @elseif($msg->status === 'sent')    &middot; &#10004;
                            @elseif($msg->status === 'read')    &middot; <span style="color:#4fc3f7;">&#10004;&#10004;</span>
                            @elseif($msg->status === 'failed')  &middot; <span style="color:#dc3545;">Failed</span>
                            @endif
                        @endif
                    </div>
                </div>
        ...
        {{-- Reply bar --}}
        <div class="reply-bar">
            <form method="POST" action="{{ route('whatsapp.support.reply', $conversation) }}" style="display:contents;" id="replyForm">
                @csrf
    ...
    <div>
        ...
        <div class="sidebar-card">
            ...
            <div class="info-row">
                ...
                <span class="value">{{ $conversation->customer_phone }}</span>
            </div>
            @if($conversation->customer)
            <div class="info-row">
                <span class="label">Customer</span>
                <span><a href="{{ route('customers.show', $conversation->customer) }}" style="color:#667eea;">{{ $conversation->customer->name }}</a></span>
            </div>
            <div class="info-row">
                <span class="label">Email</span>
                <span class="value" style="font-size:.8rem;">{{ $conversation->customer->email ?? '—' }}</span>
            </div>
            @endif
            <div class="info-row">
            ...
            <div class="info-row">
                ...
                <span class="value">{{ $conversation->created_at->format('d M Y') }}</span>
            </div>
        </div>
        ...
        <div class="sidebar-card">
            <h4>&#9881; Manage</h4>
            <form method="POST" action="{{ route('whatsapp.support.update', $conversation) }}">
                @csrf @method('PATCH')
                ...
                <div class="form-group">
                    ...
                    <select name="assigned_to">
                        ...
                        @foreach($staffList as $staff)
                            <option value="{{ $staff->id }}" {{ $conversation->assigned_to == $staff->id ? 'selected' : '' }}>
                                {{ $staff->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea name="notes" placeholder="Private notes...">{{ $conversation->notes }}</textarea>
                </div>
        ...
        <div class="sidebar-card">
            ...
            @endphp
            @foreach($quick as $qr)
            <button onclick="fillReply(this)" data-msg="{{ $qr }}"
