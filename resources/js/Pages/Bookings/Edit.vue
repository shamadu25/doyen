...
const form = useForm({
    customer_id: props.booking.customer_id || ('' as string | number),
    vehicle_id: props.booking.vehicle_id || ('' as string | number),
    assigned_to: props.booking.assigned_to || ('' as string | number),
    appointment_type: props.booking.appointment_type || '',
    scheduled_date: props.booking.scheduled_date ? props.booking.scheduled_date.split(' ')[0] : '',
    scheduled_time: props.booking.scheduled_date ? props.booking.scheduled_date.split(' ')[1]?.substring(0, 5) : '',
...
function submit() {
    ...
    const scheduledDateTime = `${form.scheduled_date} ${form.scheduled_time}:00`
    ...
    })).put(route(`/bookings/${props.booking.id}`))
}
</script>

<template>
    <Head :title="`Edit Booking ${booking.reference}`" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route(`/bookings/${booking.id}`)" class="text-gray-500 hover:text-gray-700 transition">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </Link>
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit Booking — {{ booking.reference }}</h2>
            </div>
        </template>
            ...
            <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="flex items-center justify-end gap-3">
                        <Link :href="route(`/bookings/${booking.id}`)" class="rounded-xl border border-gray-300 bg-white px-6 py-2.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition">
                            Cancel
                        </Link>
                        <button
                            type="submit"
