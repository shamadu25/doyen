        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
                <div class="p-4 border-b border-gray-200 flex flex-wrap gap-3">
                    <input v-model="search" type="text" placeholder="Search payments..." class="rounded-lg border-gray-300 text-sm focus:border-electric-600 focus:ring-electric-600 flex-1 min-w-0" />
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="hidden sm:table-cell px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Invoice</th>
                                ...
                                <th class="hidden md:table-cell px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Method</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="pay in payments.data" :key="pay.id" class="hover:bg-gray-50">
                                <td class="px-4 py-3 whitespace-nowrap">
                                    ...
                                    <p class="sm:hidden text-xs text-gray-500 mt-0.5">{{ pay.customer?.first_name }} {{ pay.customer?.last_name }}</p>
                                </td>
                                ...
                                <td class="hidden sm:table-cell px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ pay.customer?.first_name }} {{ pay.customer?.last_name }}</td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-right font-medium text-green-600">{{ fmt(pay.amount) }}</td>
                                <td class="hidden md:table-cell px-4 py-3 whitespace-nowrap text-sm text-gray-600 capitalize">{{ (pay.payment_method || '').replace('_',' ') }}</td>
                                <td class="hidden md:table-cell px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ pay.payment_date ? new Date(pay.payment_date).toLocaleDateString('en-GB') : '-' }}</td>
                                <td class="px-4 py-3 whitespace-nowrap"><StatusBadge :status="pay.status || 'completed'" size="sm" /></td>
