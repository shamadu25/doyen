        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
                <div class="p-4 border-b border-gray-200 flex flex-wrap gap-3">
                    <input v-model="search" type="text" placeholder="Search invoices..." class="rounded-lg border-gray-300 text-sm focus:border-electric-600 focus:ring-electric-600 w-64" />
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">VAT</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="inv in invoices.data" :key="inv.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ inv.customer?.first_name }} {{ inv.customer?.last_name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ inv.invoice_date ? new Date(inv.invoice_date).toLocaleDateString('en-GB') : '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-600">{{ fmt(inv.subtotal) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-600">{{ fmt(inv.vat_amount || inv.tax_amount) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-gray-900">{{ fmt(inv.total) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap"><StatusBadge :status="inv.status" size="sm" /></td>
