...
const iconPaths: Record<string, string> = {
    'chart-bar': '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13h2v8H3zM9 9h2v12H9zM15 5h2v16h-2zM21 1h2v20h-2z"/>',
    ...
    'globe': '<path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253M2.284 14.253A8.959 8.959 0 012 12c0-.778.099-1.533.284-2.253"/>',
    'support': '<path stroke-linecap="round" stroke-linejoin="round" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"/>',
    ...
    'default': '<path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>',
}
    ...
    return defineComponent({
        render() {
            return h('svg', {
                ...
                viewBox: '0 0 24 24',
                'stroke-width': '1.5',
                innerHTML: iconPaths[name] || iconPaths['default'],
            })
        }
    })
}
...
const navigation = [
    ...
    { name: 'Job Cards', href: '/job-cards', icon: 'clipboard-list' },
    { name: 'Quotes', href: '/quotes', icon: 'document-duplicate' },
    { name: 'MOT Tests', href: '/mot-tests', icon: 'shield-check' },
    { name: 'Health Checks', href: '/health-checks', icon: 'clipboard-check' },
    { name: 'Invoices', href: '/invoices', icon: 'document-text' },
    { name: 'Payments', href: '/payments', icon: 'credit-card' },
    { name: 'Inventory', href: '/inventory', icon: 'cube' },
    { name: 'Suppliers', href: '/suppliers', icon: 'building-storefront' },
    { name: 'ECU Jobs', href: '/ecu-jobs', icon: 'cpu' },
    { name: 'Services', href: '/services', icon: 'tag' },
    { name: 'Staff', href: '/staff', icon: 'user-group' },
    { name: 'Reports', href: '/reports', icon: 'chart-pie' },
    { name: 'Website', href: '/website', icon: 'globe' },
    { name: 'Support Tickets', href: '/tickets', icon: 'support' },
...
<template>
    <div class="min-h-screen bg-gray-50">
        ...
        <div v-if="mobileMenuOpen" class="fixed inset-0 z-40 bg-gray-600/75 lg:hidden" @click="mobileMenuOpen = false" />

        <!-- Sidebar -->
        <aside :class="[
            'fixed inset-y-0 left-0 z-50 flex flex-col bg-navy-950 transition-all duration-300',
            sidebarOpen ? 'w-64' : 'w-20',
            mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        ]">
            <!-- Logo -->
            <div class="flex h-16 items-center justify-between px-4 border-b border-navy-700">
                <Link :href="route('/dashboard')" class="flex items-center space-x-3">
                    <div class="w-8 h-8 rounded-lg bg-electric-600 flex items-center justify-center">
                        <span class="text-white font-bold text-sm">DA</span>
                    </div>
                    <span v-if="sidebarOpen" class="text-white font-semibold text-lg tracking-tight">Doyen Auto Services</span>
                </Link>
            ...
            <div class="border-t border-navy-700 p-4">
                <div class="flex items-center">
                    <div class="w-8 h-8 rounded-full bg-electric-600 flex items-center justify-center">
                    ...
                    <button v-if="sidebarOpen" @click="logout" class="ml-2 text-slate-400 hover:text-white" title="Logout">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <header class="sticky top-0 z-30 bg-white border-b border-gray-200">
                <div class="flex h-16 items-center justify-between px-4 sm:px-6">
                    ...
                    <div class="flex items-center space-x-4">
                        <span class="text-sm text-gray-500">{{ new Date().toLocaleDateString('en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) }}</span>
                    </div>
                </div>
            </header>
